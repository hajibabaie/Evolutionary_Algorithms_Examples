...
class Data:
    ...
    def create_and_save(self):
        ...
        a = int(np.sum(customers_demand) / self._number_of_suppliers)

        suppliers_capacity = np.random.uniform(a, 1.2 * a, (1, self._number_of_suppliers))

        suppliers_fixed_costs = 1000 + 50 * suppliers_capacity

        os.makedirs("./data", exist_ok=True)
        np.save("./data/distances.npy", distances)
        np.save("./data/customers_demand.npy", customers_demand)
        np.save("./data/suppliers_fixed_costs.npy", suppliers_fixed_costs)
        np.save("./data/suppliers_capacity.npy", suppliers_capacity)
    ...
    def load():
        ...
        out = {
            ...
            "suppliers_fixed_cost": np.load("./data/suppliers_fixed_costs.npy"),
